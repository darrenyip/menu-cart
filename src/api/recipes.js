...
export const recipesApi = {
   ...
   */
  async getOne(id) {
    const recipe = await pb.collection(COLLECTION).getOne(id, { requestKey: null })
    // 获取菜谱的原料
  ...
  async update(id, data, materials = []) {
    ...
    for (const old of oldMaterials) {
      await pb.collection(MATERIALS_COLLECTION).delete(old.id, { requestKey: null })
    }
  ...
  async delete(id) {
    ...
    for (const material of materials) {
      await pb.collection(MATERIALS_COLLECTION).delete(material.id, { requestKey: null })
    }
